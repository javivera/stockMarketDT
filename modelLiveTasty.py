...
def modelLive(symbol:str,interval:str,live=0,model=0):
    current_date = datetime.now().date().strftime("%Y-%m-%d")
    model_number = f'model-{model}'

    print('Creating folders')
    treeCreator(symbol,interval)
    csv_file_path = f'live/market/{symbol}/{interval}/{current_date}.csv'
    ...
    while True:
        ...
        # if True:
            ...
            start_time = datetime.now() - timedelta(days=10)  # 1 month ago
            ...
            print(df_prep.Pred.iloc[-1])
            
            df['Predicted'] = df_prep['Pred'].iloc[-1]
            # price = float(df.iloc[-1,1])
            last_predict = df_prep['Pred'].iloc[-1]
            # print(last_predict)
...
def treeCreator(symbol,interval):
    # if os.path.exists(f"live/{symbol}/{interval}/"):
        # shutil.rmtree(f"live/{symbol}/{interval}/")

    if not os.path.exists("live"):
        os.mkdir("live")

    if not os.path.exists(f"live/market/{symbol}"):
        os.mkdir(f"live/market/{symbol}")

    if not os.path.exists(f"live/market/{symbol}/{interval}"):
        os.mkdir(f"live/market/{symbol}/{interval}")

    if not os.path.exists(f"live/market/{symbol}/{interval}/model-1"):
        os.mkdir(f"live/market/{symbol}/{interval}/model-1")

    # if not os.path.exists(f"live/{symbol}/{interval}/model-1/graphs"):
        # os.mkdir(f"live/{symbol}/{interval}/model-1/graphs")

    # if not os.path.exists(f"live/market/{symbol}/{interval}/model-1/data"):
        # os.mkdir(f"live/market/{symbol}/{interval}/model-1/data")
...
def earningsCalculator(symbol,interval,csv_file_path):
    ...
    fig.write_html(f'live/market/{symbol}/{interval}/{current_date}-earnings_chart.html', auto_open=False)
    try:
        return df
    except:
        raise Exception
